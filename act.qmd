...
There are `r comma(nrow(act.original), accuracy = 1)` observations in the dataset. Each observation is an ACT test taken by a student in Southern Minnesota. The unique feature here is the time it was taken. This means that a student could have taken the ACT twice and their PersonID would show up as two observations.
